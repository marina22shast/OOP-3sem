...
partial class BoolVector
{
    ...
    public bool Equals(BoolVector v2)//v2 обозначает вектор,значит параметром переходит просто вектор
    {
    ...
    new public static long GetHashCode()
    {
        ...
        //Представляет текущее время ,число тактов возвращает, добавим случайное значение, 
        //чтобы наш хеш был уникален для объектов созданных в одно и то же время с точностью до миллисекунды
        ...
        //Next-Возвращает случайное целое число.
        return result;
    }
    ...
    {
        // метод через ref-параметр (ссылочный) возвращает строковое представление ID,
        // через входной параметр out-параметр возвращает числовое значение ID
